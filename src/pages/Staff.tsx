...
export function Staff() {
  const { staff, loading, error, selectedCharacter } = useHarryPotter()
  ...
  const sortedStaff = selectedCharacter
    ? [
        selectedCharacter,
        ...filteredStaff.filter((c) => c.id !== selectedCharacter.id),
      ]
    : filteredStaff
              ...
              {currentItems.length === 0 ? (
                ...
                currentItems.map((staffMember) => (
                  <TableRow key={staffMember.id}>
                    ...
                    <TableCell>{staffMember.house || 'N/A'}</TableCell>
                    <TableCell>{staffMember.actor || 'N/A'}</TableCell>
                    <TableCell>
                      <Button
                        variant='outline'
                        onClick={() => navigate(`/character/${staffMember.id}`)}
                        className='w-full'
                      >
                        See more
                      </Button>
            ...
            currentItems.map((staffMember) => (
              ...
              >
                <div className='space-y-2'>
                  <div className='font-bold'>{staffMember.name || 'N/A'}</div>
                  <div>{staffMember.house || 'N/A'}</div>
                  <div>{staffMember.actor || 'N/A'}</div>
                  <Button
                    variant='outline'
                    onClick={() => navigate(`/character/${staffMember.id}`)}
                    className='w-full'
